...
const PendingChanges = {
  ...
  async _adminBypass(table, record, isNew) {
    ...
    if (table === 'workRequests') {
      ...
      let wr = null;
      if (isNew) {
        const res = await api.workRequests.create(cleanRecord);
        wr = res?.data || null;
      } else {
    ...
    if (table === 'tasks') {
      ...
      if (isNew) {
        ...
        const res = await api.workRequests.updateTask(wrId, record.id, cleanRecord);
        task = res?.data || null;
      }
      return task;
    }
  ...
  canApproveChange(pc) {
    if (!pc) return false;
    if (pc.submittedBy === Auth.user?.id && Auth.user?.role !== 'Admin') return false;
