...
const Billing = {
  ...
  async _withArchiveLock(fn) {
    if (this._archiveRestoreLock) {
      Workflow.showMessage(
        "Action in progress",
        "Please wait for the current archive/restore action to finish.",
        "info",
      );
      return;
    }
  ...
  _entityMatches(invEntity, entity) {
    ...
    if (entity === "ALL") {
      return (
        Auth.user?.entities?.map((e) => e.toUpperCase()).includes(u) || true
      );
    }
  ...
  _isActiveInvoice(inv, entity) {
    ...
    if (
      ...
      (Auth.user?.departments || []).includes("Accounting")
    ) {
      allowedStatuses.push("Draft", "Pending");
    }
    return (
      this._entityMatches(e, entity) &&
  ...
  async _loadInvoices(loadGen, { merge = false } = {}) {
    ...
    try {
      ...
      const invoices = (res.data || []).map((inv) =>
        this.normalizeInvoice(inv),
      );
      invoices.forEach((inv) => {
        const existing = this._detailCache[inv.id];
        if (
          existing &&
          existing.updatedAt &&
          inv.updatedAt &&
          new Date(existing.updatedAt) > new Date(inv.updatedAt)
        ) {
          return;
        }
      ...
      if (Array.isArray(this._listCache) && this._listCacheEntity === entity) {
        const existingMap = new Map(
          this._listCache.map((inv) => [inv.id, inv]),
        );
        const isSkipActive =
          this._activeSkipGeneration > 0 &&
          this._activeSkipGeneration === this._skipFetchGeneration;
        ...
        const localItems = Array.isArray(this._listCache)
          ? this._listCache
          : [];
        this._listCache = invoices;
        localItems.forEach((localInv) => {
          const existingInNew = this._listCache.find(
            (inv) => inv.id === localInv.id,
          );
          if (existingInNew) {
            const localNewer =
              localInv.updatedAt &&
              existingInNew.updatedAt &&
              new Date(localInv.updatedAt) > new Date(existingInNew.updatedAt);
            if (localNewer) {
              Object.assign(existingInNew, localInv);
            }
          } else if (!this._isTempId(localInv.id)) {
  ...
  async backgroundRefresh() {
    ...
    const loadGen = ++this._listCacheGeneration;
    this._backgroundPromise = this._loadInvoices(loadGen, {
      merge: true,
    }).finally(() => {
    ...
    this._postMutationTimer = setTimeout(() => {
      ...
      if (!skipping) {
        this.backgroundRefresh().catch((err) => {
          if (!isAbortError(err))
            console.warn("Billing post-mutation refresh failed", err);
        });
      }
    }, 600);
  },
  ...
  getInvoiceById(id) {
    if (!id) return null;
    if (this._detailCache[id]) return deepClone(this._detailCache[id]);
    if (Array.isArray(this._listCache)) {
      const inv = this._listCache.find((i) => i.id === id);
      if (inv) return deepClone(inv);
    }
    return null;
  },
  ...
  _invalidateRelatedCaches(record) {
    ...
    if (record?.workRequestId) {
      if (window.apiClient?.workRequestCache?.getById) {
        const wr = window.apiClient.workRequestCache.getById(
          record.workRequestId,
        );
        if (wr) {
          if (!Array.isArray(wr.linkedInvoiceIds)) wr.linkedInvoiceIds = [];
          if (!wr.linkedInvoiceIds.includes(record.id))
            wr.linkedInvoiceIds.push(record.id);
        }
      ...
      if (typeof WorkflowData !== "undefined") {
        if (typeof WorkflowData.getWorkRequestById === "function") {
          ...
          if (wfWr) {
            if (!Array.isArray(wfWr.linkedInvoiceIds))
              wfWr.linkedInvoiceIds = [];
            if (!wfWr.linkedInvoiceIds.includes(record.id))
              wfWr.linkedInvoiceIds.push(record.id);
          }
        }
        if (
          typeof WorkflowData.invalidateRelatedForWorkRequest === "function"
        ) {
    ...
    if (typeof Dashboard !== "undefined") {
      if (typeof Dashboard.invalidateCache === "function")
  ...
  _invalidateCountsAndSidebar() {
    ...
    if (
      ...
      typeof App.updateSidebarNotifications === "function"
    ) {
  ...
  _updateCounts(activeDelta = 0, archivedDelta = 0) {
    ...
    this._counts.archived = Math.max(
      0,
  ...
  async _fetchInvoiceAndRerender(id) {
    ...
    try {
      ...
      if (res?.data) {
        this._detailCache[id] = {
          ...this.normalizeInvoice(res.data),
          isFullDetail: true,
        };
  ...
  async _optimisticUpdate(id, patch, apiCall, errorTitle = "Error") {
    if (this._isTempId(id)) {
      Workflow.showMessage(
        "Saving...",
        "Please wait for the record to finish saving.",
        "info",
      );
    ...
    const wasActive = snapshot
      ? this._isActiveInvoice(snapshot, entity)
      : false;
    const wasArchived = snapshot
      ? this._isArchiveInvoice(snapshot, entity)
      : false;
  ...
  async _loadPrefilledOpReq() {
    ...
    try {
      const res = await window.apiClient.operationsRequests.get(
        this.prefilledRequestId,
      );
      this._prefilledOpReq = res.data || null;
    } catch (err) {
      console.error(
        "Failed to load prefilled operations request",
        this.prefilledRequestId,
        err,
      );
      this._prefilledOpReq = null;
  ...
  async loadCounts(force = false) {
    ...
    try {
      ...
      this._counts = {
        ...(res?.data || {}),
        templates: res?.data?.templates || 0,
      };
      this._countsEntity = entity;
    } catch (err) {
  ...
  async loadRejectedCount(force = false) {
    ...
    if (
      ...
      this._counts &&
      this._countsEntity === entity &&
      typeof this._counts.rejected === "number"
    ) {
    ...
    try {
      const [pendingRes, opReqRes] = await Promise.all([
        window.apiClient.admin.listPendingApprovals({
          status: "rejected",
          tableName: "invoices",
        }),
        window.apiClient.operationsRequests.list({
          status: "rejected",
          type: "billing",
        }),
      ]);
    ...
    const changed =
      !this._counts ||
      this._countsEntity !== entity ||
      this._counts.rejected !== rejected;
    this._counts = {
      ...(this._counts || {}),
      rejected,
  ...
  async render(routeId) {
    ...
    if (this.view === "detail" && this.detailId) {
      ...
      (async () => {
        try {
          if (
            !this._detailCache[this.detailId] ||
            !this._detailCache[this.detailId].isFullDetail
          ) {
          ...
          if (inv && inv.status !== "Draft" && inv.status !== "Pending") {
            const noLogoLabel = el("label", {
              style:
                "margin-right:12px; font-size:0.8125rem; display:inline-flex; align-items:center; gap:6px; cursor:pointer; color:var(--color-text-muted);",
            });
            const noLogoCheckbox = el("input", {
              type: "checkbox",
              id: "print-no-logo",
            });
            ...
            const genVouchBtn = el("button", {
              class: "btn btn-secondary btn-sm",
              text: "Print Voucher (No Header)",
              style: "margin-right:8px;",
            });
            genVouchBtn.addEventListener("click", () =>
              this.generateVoucher(inv),
            );
    ...
    if (this.view === "form") {
      const userViewMode = window.SidePaneInstance
        ? window.SidePaneInstance.resolveMode({ viewContext: "invoice-form" })
        : "side-peek";
      ...
      const viewSwitcher = buildFormViewSwitcher({
        ...
        onNewTab: () => {
          window.open(
            location.origin + location.pathname + fullPageRoute,
            "_blank",
            "noopener,noreferrer",
          );
        },
      ...
      container.appendChild(
        buildFormBreadcrumb({
          ...
          currentText: isNew
            ? "New Invoice"
            : inv?.invoiceNumber || "Edit Invoice",
          viewSwitcher,
          actions: [
            {
      ...
      const fullPageRoute = isNew
        ? "#billing/templateForm/new"
        : `#billing/templateForm/${this.templateEditingId}`;
      const viewSwitcher = buildFormViewSwitcher({
        currentMode: PaneMode.FULL_PAGE,
        ...
        onSidePeek: async () => {
          ...
          const template = isNew
            ? null
            : await this.getTemplateById(this.templateEditingId);
      ...
      const isLimitedView =
        ...
        Auth.user?.departments?.includes("Documentation") ||
        (Auth.isManagerial() && Auth.user?.role !== "Admin");
      if (
        isLimitedView &&
        ["templates", "aging", "templateForm"].includes(this.view)
      ) {
        this.view = "list";
        location.hash = "#billing";
      }
    ...
    if (this.view === "form") {
      ...
      const template = !this.templateEditingId
        ...
        : await this.getTemplateById(this.templateEditingId);
      container.appendChild(
        await this.renderTemplateForm({ hideHeader: true, template }),
      );
    }
  ...
  renderTabNav() {
    ...
    const cacheActiveCount = cachedInvoices.filter((inv) =>
      this._isActiveInvoice(inv, entity),
    ).length;
    const cacheArchiveCount = cachedInvoices.filter((inv) =>
      this._isArchiveInvoice(inv, entity),
    ).length;
    const invoiceCount = cacheWarm
      ? cacheActiveCount
      : this._counts?.active || 0;
    const archiveDbCount = cacheWarm
      ? cacheArchiveCount
      ...
      tabs.push(
        {
          key: "templates",
          label: "Templates",
          icon: '<svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><rect x="3" y="3" width="18" height="18" rx="2"/><line x1="9" y1="3" x2="9" y2="21"/></svg>',
          count: templateCount,
        },
        {
          key: "aging",
    ...
    if (canCreate && canRequest) {
      ...
      primaryBtn.innerHTML =
        '<svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5" stroke-linecap="round" stroke-linejoin="round"><line x1="12" y1="5" x2="12" y2="19"/><line x1="5" y1="12" x2="19" y2="12"/></svg> New Billing';
      ...
      requestItem.innerHTML =
        '<svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M4 4h16c1.1 0 2 .9 2 2v12c0 1.1-.9 2-2 2H4c-1.1 0-2-.9-2-2V6c0-1.1.9-2 2-2z"></path><polyline points="22,6 12,13 2,6"></polyline></svg> Request Billing';
  ...
  normalizeInvoice(doc, entityCodeHint) {
    ...
    let entity =
      ...
      ["ATA", "LTA"].includes(doc.entity.toUpperCase())
        ? doc.entity
        : null) ||
    ...
    if ((!entity || entity === "ALL") && (doc.client_id || doc.clientId)) {
      const client = window.apiClient.clientCache.getById(
        doc.client_id || doc.clientId,
      );
      if (client?.entity) {
    ...
    if (
      (!entity || entity === "ALL") &&
      (doc.work_request_id || doc.workRequestId)
    ) {
      const wr = window.apiClient.workRequestCache.getById(
        doc.work_request_id || doc.workRequestId,
      );
    ...
    return {
      ...
      paidAmount:
        parseFloat(doc.amount_paid) || parseFloat(doc.paidAmount) || 0,
      balance:
        parseFloat(doc.balance) ||
        parseFloat(doc.total || 0) - parseFloat(doc.amount_paid || 0),
      ...
      lineItems: doc.line_items
        ? (doc.line_items || []).map(normLineItem)
        : (doc.lineItems || []).map(normLineItem),
  ...
  async fetchInvoices(query = {}) {
    ...
    try {
      ...
      if (
        loadGen !== this._listCacheGeneration ||
      ...
      if (isListFetch) {
        this._listCacheEntity = entity;
        invoices.forEach((inv) => {
          const existing = (this._listCache || []).find((i) => i.id === inv.id);
          if (
            existing &&
            existing.updatedAt &&
            inv.updatedAt &&
  ...
  async _loadTemplates(loadGen, { merge = false } = {}) {
    ...
    try {
      ...
      if (
        loadGen !== this._templatesGeneration ||
        Auth.activeEntity !== entity
      ) {
        return this._templates || [];
      }
      const filtered = all.filter((t) => {
        if (entity === "ALL") {
          return Auth.user.entities
            .map((e) => e.toUpperCase())
            .includes((t.entity || "").toUpperCase());
        }
  ...
  async backgroundRefreshTemplates() {
    if (this._templatesBackgroundPromise)
      return this._templatesBackgroundPromise;
    const loadGen = ++this._templatesGeneration;
    this._templatesBackgroundPromise = this._loadTemplates(loadGen, {
      merge: true,
    }).finally(() => {
      if (this._templatesGeneration === loadGen)
        this._templatesBackgroundPromise = null;
    });
    return this._templatesBackgroundPromise;
  },
  ...
  renderList() {
    ...
    if (savedFilters) {
      if (Array.isArray(savedFilters.workRequest))
        savedFilters.workRequest.forEach((v) =>
          activeFilters.workRequest.add(v),
        );
      else if (savedFilters.workRequest)
        activeFilters.workRequest.add(savedFilters.workRequest);
      if (Array.isArray(savedFilters.client))
        savedFilters.client.forEach((v) => activeFilters.client.add(v));
      else if (savedFilters.client)
        activeFilters.client.add(savedFilters.client);
      if (Array.isArray(savedFilters.employee))
        savedFilters.employee.forEach((v) => activeFilters.employee.add(v));
      else if (savedFilters.employee)
        activeFilters.employee.add(savedFilters.employee);
      if (Array.isArray(savedFilters.status))
        savedFilters.status.forEach((v) => activeFilters.status.add(v));
      else if (savedFilters.status)
        activeFilters.status.add(savedFilters.status);
      if (Array.isArray(savedFilters.date))
        savedFilters.date.forEach((v) => activeFilters.date.add(v));
    }
    ...
    const entMatches = (val) => {
      ...
      if (entity === "ALL")
        return Auth.user.entities.map((ae) => ae.toUpperCase()).includes(u);
      return u === entity.toUpperCase();
    };

    const getWorkRequestOptions = () => {
      return window.apiClient.workRequestCache
    ...
    const getEmployeeOptions = () => {
      ...
      users
        .filter((u) => {
          ...
          return entity === "ALL"
            ? userEnts.some((e) =>
                Auth.user.entities.map((ae) => ae.toUpperCase()).includes(e),
              )
            : userEnts.includes(entity.toUpperCase());
        })
      ...
      },
      {
        ...
        getName: (inv) => {
          const creator = inv.createdBy
            ? window.apiClient.userCache.getById(inv.createdBy)
            : null;
    ...
    const refresh = async () => {
      ...
      let baseInvoices = (this._listCache || []).filter((inv) =>
        this._isActiveInvoice(inv, entity),
      );

      let pendingInvs = [];
      try {
        const pendingRes = await window.apiClient.pendingApprovals.list({
          status: "pending",
          tableName: "invoices",
        });
        pendingInvs = (pendingRes.data || [])
          .filter((pc) => {
            ...
            const matchesEntity =
              entity === "ALL"
                ? Auth.user.entities
                    .map((ae) => ae.toUpperCase())
                    .includes((inv.entity || "").toUpperCase())
                : (inv.entity || "").toUpperCase() === entity.toUpperCase();
            if (!matchesEntity) return false;
            if (
              !Auth.can("billing:approve") &&
              pc.submittedBy !== Auth.user?.id
            )
              return false;
            return true;
          })
      ...
      let invoices = [...baseInvoices, ...pendingInvs].filter((inv) =>
        allowedStatuses.includes(inv.status),
      );

      if (activeFilters.workRequest.size > 0) {
        invoices = invoices.filter((inv) =>
          activeFilters.workRequest.has(inv.workRequestId),
        );
      }
      if (activeFilters.client.size > 0) {
        invoices = invoices.filter((inv) =>
          activeFilters.client.has(inv.clientId),
        );
      }
      if (activeFilters.employee.size > 0) {
        invoices = invoices.filter((inv) => {
          ...
          const wr = inv.workRequestId
            ? window.apiClient.workRequestCache.getById(inv.workRequestId)
            : null;
          const tasks = wr?.tasks || [];
          return tasks.some((t) => {
            const u = t.assigneeId
              ? window.apiClient.userCache.getById(t.assigneeId)
              : null;
            return (
              (u && activeFilters.employee.has(u.name)) ||
              activeFilters.employee.has(t.assigneeName)
            );
          });
        });
      }
      if (activeFilters.status.size > 0) {
        invoices = invoices.filter((inv) =>
          activeFilters.status.has(inv.status),
        );
      }
      if (activeFilters.date.size > 0) {
        ...
        endOfWeek.setDate(
          now.getDate() + (now.getDay() === 0 ? 0 : 7 - now.getDay()),
        );
      ...
      if (paginatedInvoices.length === 0 && hasActiveFilters && hasInvoices) {
        contentContainer.appendChild(
          renderFilterEmptyState("No invoices match your filters", null, [
            {
              text: "Clear filters",
    ...
    (async () => {
      ...
      if (!skipping) {
        this.backgroundRefresh().catch((err) => {
          if (!isAbortError(err))
            console.warn("Billing background refresh failed", err);
        });
      }
    })();
  ...
  refreshTable(container, invoices) {
    ...
    if (invoices.length === 0) {
      container.appendChild(
        renderEmptyState("No invoices found", null, { variant: "zero-state" }),
      );
      return;
    }
    ...
    const buildActions = (inv) => {
      ...
      viewBtn.addEventListener("click", (e) => {
        e.stopPropagation();
        location.hash = "#billing/detail/" + inv.id;
      });
    ...
    const columns = [
      {
        ...
        render: (inv) => {
          ...
          if (inv.workRequestId) {
            ...
            if (wr) {
              const sub = el("div", {
                style: "font-size: 0.725rem; color: var(--color-text-muted);",
              });
              let suffix = " (Entire WR)";
              if (inv.linkedTaskId) {
                const task = (wr.tasks || []).find(
                  (t) => t.id === inv.linkedTaskId,
                );
                if (task) suffix = ` (Task: ${task.title})`;
              }
              sub.appendChild(
                el("span", {
                  text: "🔗 " + wr.title + suffix,
      ...
      },
      {
        key: "total",
        label: "Total",
        render: (inv) => formatPHP(inv.total || 0),
        align: "right",
        width: "100px",
      },
      {
        key: "paid",
        label: "Paid",
        render: (inv) => formatPHP(this.getPaidAmount(inv)),
        align: "right",
        width: "100px",
      },
      {
        key: "balance",
        label: "Balance",
        render: (inv) => formatPHP((inv.total || 0) - this.getPaidAmount(inv)),
        align: "right",
        width: "100px",
      },
      {
        key: "status",
        label: "Status",
        render: (inv) => this.statusBadge(inv.status),
        width: "110px",
      },
      {
        key: "actions",
        label: "Actions",
        render: (inv) => buildActions(inv),
        class: "dt-actions-col",
        width: "180px",
      },
    ];
    ...
    const tableView = DataTable.render({
      ...
      bulkActions: isOperations
        ? null
        : (ids) => {
            const rows = ids
              .map((id) => invoices.find((inv) => inv.id === id))
              .filter(Boolean);
            const canArchive = rows.filter(
              (inv) => inv.status === "Paid" && !inv.archived,
            ).length;
            const canTrash = rows.filter(
              (inv) => inv.status === "Draft" && Auth.can("billing:edit"),
            ).length;
            const actions = [];
            if (canArchive > 0) {
              actions.push({
      ...
      rowId: (inv) => inv.id,
      onRowClick: (inv) => {
        if (!inv.pendingChangeId) location.hash = "#billing/detail/" + inv.id;
      },
    });
  ...
  getBoardColumns() {
    ...
    const isOperations =
      ...
      departments.includes("Documentation") ||
      (Auth.isManagerial() && role !== "Admin");
  ...
  refreshBoard(
    ...
    if (invoices.length === 0) {
      container.appendChild(
        renderEmptyStateV2({
          ...
          body: "Create a billing invoice to start tracking payments.",
        }),
      );
      return;
    }
    ...
    boardPhases.forEach((phase) => {
      const colInvs = invoices.filter(
        (inv) =>
          phase.statuses.includes(inv.status) &&
          !inv.pendingChangeId &&
          !inv.archived &&
          inv.status !== "Cancelled",
      );
      ...
      const colPendingInvs = invoices.filter(
        (inv) => phase.statuses.includes(inv.status) && inv.pendingChangeId,
      );
    ...
    const boardDrag = {
      ...
      canDrag: (inv) => {
        ...
        const canManage =
          canEdit ||
          Auth.can("billing:approve") ||
          Auth.can("billing:mark_paid") ||
          Auth.can("billing:release") ||
          Auth.isManagerial();
      ...
      onDropDenied({ item, targetStatus }) {
        ...
        if (item.status === "Draft" && targetStatus !== "Pending") {
          Workflow.showMessage(
            "Submission Required",
            `Invoice "${item.invoiceNumber}" must be submitted for approval (Pending status) before it can be advanced.`,
            "warning",
          );
          return;
        }

        if (targetStatus !== "Partially Paid" && targetStatus !== "Paid")
          return;
        const paid = self.getPaidAmount(item);
        const isReleased = [
          "Sent",
        ...
        if (!isReleased && !Auth.isManagerial()) {
          Workflow.showMessage(
            ...
            `Invoice "${item.invoiceNumber}" is in ${item.status} status and has not been released yet. Payments can only be recorded after an invoice is approved and released to the client.`,
            "warning",
          );
          return;
        }
        ...
        if (targetStatus === "Partially Paid") {
          if (paid <= 0) {
            Workflow.showMessage(
              "Payment Required",
              `Invoice "${item.invoiceNumber}" cannot be marked Partially Paid — no payments have been recorded.`,
              "warning",
            );
          } else if (paid >= item.total) {
            Workflow.showMessage(
              "Already Fully Paid",
              `Invoice "${item.invoiceNumber}" has payments totaling ${formatPHP(paid)}. Use the Paid status instead.`,
              "warning",
            );
          }
          ...
          if (item.total <= 0) {
            Workflow.showMessage(
              "Invalid Invoice",
              `Invoice "${item.invoiceNumber}" has no billable amount and cannot be marked Paid.`,
              "warning",
            );
          } else if (paid <= 0) {
            Workflow.showMessage(
              "Payment Required",
              `Invoice "${item.invoiceNumber}" cannot be marked Paid — no payments have been recorded.`,
              "warning",
            );
          } else if (paid < item.total) {
            ...
            Workflow.showMessage(
              "Balance Remaining",
              `Invoice "${item.invoiceNumber}" still has a balance of ${formatPHP(balance)}. Record the remaining payment before marking Paid.`,
              "warning",
            );
          }
        }
      ...
      onDrop({ item, targetStatus, newOrder, fromStatus }) {
        if (self._isTempId(item.id)) {
          Workflow.showMessage(
            "Saving...",
            "Please wait for the invoice to finish saving before moving it.",
            "info",
          );
          return;
        }
        ...
        if (
          item.status === "Draft" &&
          ["Sent", "Paid", "Partially Paid"].includes(nextStatus)
        ) {
          Workflow.showMessage(
            "Submission Required",
            `Invoice "${item.invoiceNumber}" must be submitted for approval (Pending status) before it can be advanced.`,
        ...
        if (
          item.status === "Pending" &&
          ["Paid", "Partially Paid"].includes(nextStatus)
        ) {
          Workflow.showMessage(
            "Invoice Not Released",
            `Invoice "${item.invoiceNumber}" must be released before recording payments.`,
            "warning",
          );
          return;
        }
        ...
        if (targetStatus === "Approved" && !Auth.can("billing:approve")) {
          Workflow.showMessage(
            "Permission Denied",
            "Only users with approval rights can approve invoices.",
            "danger",
          );
          return;
        }
        ...
        if (
          ...
          Workflow.showConfirm(
            "Request Paid Phase Routing",
            `Submit request to route invoice "${item.invoiceNumber}" (${formatPHP(item.total)}) to Paid phase for Admin approval?`,
            () => {
              Workflow.runBlockingArchiveAction({
                ...
                apiCall: async () => {
                  ...
                  return await window.apiClient.operationsRequests.create(
                    record,
                  );
                },
                ...
                onAfterConfirm: async () => {
                  if (
                    ...
                    typeof Users.invalidateMyRequestsCount === "function"
                  ) {
                    Users.invalidateMyRequestsCount();
                  }
                  if (
                    typeof window.apiClient?.operationsRequests
        ...
        if (item.pendingChangeId) {
          Workflow.showMessage(
            "Pending Approval",
            `Invoice "${item.invoiceNumber}" is pending administrative approval and cannot be moved.`,
            "warning",
          );
          return;
        }
        ...
        if (fromStatus === "Draft" && targetStatus !== "Pending") {
          ...
          if (!hasItems && (!item.total || item.total <= 0)) {
            Workflow.showMessage(
              "Incomplete Invoice",
              "Cannot advance — invoice has no line items or amount.",
              "warning",
            );
            return;
          }
        ...
        const applyMove = async () => {
          ...
          const runResult = await Workflow.runBlockingArchiveAction({
            title:
              nextStatus === "Release Pending Approval"
                ? "Submitting for Release Approval"
                : "Routing Invoice",
            message: `Please wait while "${item.invoiceNumber}" is being moved to ${nextStatus}...`,
            apiCall: async () => {
              self._updateCachedItem(item.id, {
                status: nextStatus,
                boardOrder: newOrder,
              });
              const res = await window.apiClient.invoices.update(item.id, {
        ...
        if (["Approved", "Sent", "Paid"].includes(targetStatus)) {
          const labels = {
            Approved: {
              msg: `Approve invoice "${item.invoiceNumber}" (${formatPHP(item.total)})?`,
              type: "success",
            },
            Sent: {
              msg: canReleaseDirectly
                ? `Release invoice "${item.invoiceNumber}" to client?`
                : `Submit invoice "${item.invoiceNumber}" for release approval?`,
              type: "success",
            },
            Paid: {
              msg: `Mark invoice "${item.invoiceNumber}" (${formatPHP(item.total)}) as fully Paid?`,
              type: "success",
            },
          };
  ...
  statusBadge(status) {
    ...
    return el("span", {
      class: "badge " + (map[status] || ""),
      text: this.getInvoiceDisplayStatus(status),
    });
  },
  ...
  async renderForm(invoiceId = null) {
    ...
    const prefill =
      ...
      (opReq
        ? {
            workRequestId: opReq.workRequestId || opReq.work_request_id,
            clientId: opReq.clientId || opReq.client_id,
            linkedTaskId: opReq.linkedTaskId || opReq.linked_task_id,
          }
        : null);
    ...
    const form = el("form", {
      id: "invoice-form",
      class: "form-stacked notion-form",
    });
    ...
    clientGroup.appendChild(
      el("label", {
        html: '<svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M17 21v-2a4 4 0 0 0-4-4H5a4 4 0 0 0-4 4v2"/><circle cx="9" cy="7" r="4"/><path d="M23 21v-2a4 4 0 0 0-3-3.87"/><path d="M16 3.13a4 4 0 0 1 0 7.75"/></svg> Client',
      }),
    );
    const clientSelAttrs = {
      name: "clientId",
      required: true,
    ...
    if (existingWr && !activeWrIds.has(existingWr.id)) {
      const inactiveSuffix = existingWr.archived
        ? " [Archived]"
        : existingWr.status === "Cancelled"
          ? " [Cancelled]"
          : "";
      wrSel.appendChild(
        el("option", {
          value: existingWr.id,
          text: existingWr.title + inactiveSuffix,
          selected: true,
        }),
      );
    }
    wrs.forEach((wr) => {
      ...
      else if (!inv && prefill && prefill.workRequestId === wr.id)
        opt.selected = true;
    ...
    taskGroup.appendChild(
      el("label", {
        html: '<svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M9 11l3 3L22 4"/><path d="M21 12v7a2 2 0 01-2 2H5a2 2 0 01-2-2V5a2 2 0 012-2h11"/></svg> Task',
      }),
    );
    ...
    const updateTasks = () => {
      ...
      taskSel.appendChild(
        el("option", { value: "", text: "— Whole Project —" }),
      );
      ...
      if (wrId) {
        ...
        tasks.forEach((t) => {
          ...
          else if (!inv && prefill && prefill.linkedTaskId === t.id)
            opt.selected = true;
    ...
    issueDateProp.appendChild(
      el("label", {
        html: '<svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><rect x="3" y="4" width="18" height="18" rx="2" ry="2"/><line x1="16" y1="2" x2="16" y2="6"/><line x1="8" y1="2" x2="8" y2="6"/><line x1="3" y1="10" x2="21" y2="10"/></svg> Issue Date',
      }),
    );
    issueDateProp.appendChild(
      el("input", {
    ...
    numProp.appendChild(
      el("label", {
        html: '<svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><rect x="4" y="5" width="16" height="16" rx="2"/><line x1="16" y1="3" x2="16" y2="7"/><line x1="8" y1="3" x2="8" y2="7"/><line x1="4" y1="11" x2="20" y2="11"/></svg> Invoice Number',
      }),
    );
    const numInput = el("input", {
      ...
      name: "invoiceNumber",
      class: "notion-prop-input",
      value: inv ? inv.invoiceNumber : "",
      readonly: true,
    });
    if (!inv) {
      this.nextInvoiceNumber(entity, this.currentListPage || 1)
        .then((n) => {
          numInput.value = n;
        })
        .catch(() => {});
    ...
    totals.appendChild(
      el("div", { class: "notion-total-row" }, [
        el("span", { text: "Subtotal" }),
        el("span", { id: "inv-subtotal", text: "₱0.00" }),
      ]),
    );
    totals.appendChild(
      el("div", { class: "notion-total-row notion-total-grand" }, [
        el("span", { text: "Total" }),
    ...
    form.addEventListener("submit", (e) => {
      e.preventDefault();
      this.submitForm(form).catch((err) =>
  ...
  addLineItemRow(container, item) {
    ...
    const descIn = el("input", {
      ...
      class: "item-desc notion-line-item-desc",
    ...
    const amtIn = el("input", {
      type: "number",
      placeholder: "0.00",
      class: "item-amt notion-line-item-amt",
      value: item?.amount || "",
      min: 0,
      step: 0.01,
    });
  ...
  nextInvoiceNumber(entity, page = 1) {
    return typeof Utils !== "undefined" &&
      typeof Utils.nextInvoiceNumber === "function"
  ...
  async submitForm(form) {
    ...
    if (isNew) {
      const wrName = data.workRequestId
        ? window.apiClient.workRequestCache.getById(data.workRequestId)
            ?.title || ""
        : "";
      ...
      await Workflow.runBlockingArchiveAction({
        ...
        apiCall: async () => {
          if (record.status === "Draft" || !requiresApproval) {
            const res = await window.apiClient.invoices.update(
              record.id,
              apiPayload,
            );
            serverRecord = this.normalizeInvoice(res.data);
        ...
        successMessage: () => {
          ...
          return isApproved
            ? "Invoice " +
                (serverRecord?.invoiceNumber || record.invoiceNumber) +
                " has been updated successfully." +
                linkMsg
            : "Invoice " +
                record.invoiceNumber +
                " update request has been submitted for Admin approval.";
        },
        ...
        onSuccess: async (res) => {
          ...
          if (
            typeof window.apiClient?.invoices?.invalidateCounts === "function"
          ) {
            window.apiClient.invoices.invalidateCounts();
          }
          if (
  ...
  async showRequestInvoiceModal() {
    ...
    try {
      const pendingRes = await window.apiClient.operationsRequests.list({
        status: "pending",
        type: "billing",
      });
    ...
    const pendingWrIds = new Set(
      pendingBillingRequests
        .map((r) => r.work_request_id || r.workRequestId)
        .filter(Boolean),
    );

    const wrapper = el("div", {
      style:
        "display: flex; flex-direction: column; gap: var(--spacing-md); min-width: 420px; max-width: 500px;",
    });
    ...
    wrSelect.appendChild(
      el("option", { value: "", text: "— Select Work Request —" }),
    );
    const extraWrId =
      this.pendingPrefill?.workRequestId || this.prefilledWrId || null;
    const extraWr = extraWrId
      ? window.apiClient.workRequestCache.getById(extraWrId)
      : null;
    if (extraWr && !activeWrIds.has(extraWr.id)) {
      const client = window.apiClient.clientCache.getById(extraWr.clientId);
      const inactiveSuffix = extraWr.archived
        ? " [Archived]"
        : extraWr.status === "Cancelled"
          ? " [Cancelled]"
          : "";
      wrSelect.appendChild(
        el("option", {
          value: extraWr.id,
          text: `${extraWr.title} — ${client?.name || "—"}${inactiveSuffix}`,
          selected: true,
        }),
      );
    }
    ...
    amtIn.addEventListener("focus", () => {
      const n = parseFloat(String(amtIn.value).replace(/[₱$,\s]/g, "")) || 0;
      amtIn.value = n > 0 ? String(n) : "";
    });
    ...
    fileGroup.appendChild(
      el("label", { text: "Proof of Completion (optional)" }),
    );
    ...
    const notesArea = el("textarea", {
      ...
      placeholder: "e.g. Requesting milestone Downpayment billing...",
    });
    ...
    const footer = el(
      ...
      {
        style:
          "display: flex; justify-content: flex-end; gap: 8px; margin-top: var(--spacing-md); border-top: 1px solid var(--color-border); padding-top: var(--spacing-sm);",
      },
      [
        el("button", {
          id: "btn-cancel-opreq",
    ...
    const overlay = Workflow.showModal(
  ...
  renderDetail() {
    ...
    const statusWrap = el("div", {
      style:
        "display:flex; gap:8px; align-items:center; margin-bottom: var(--spacing-lg);",
    });
    ...
    if (inv.status === "Draft" && inv.rejectionReason) {
      const rejBanner = el("div", {
        ...
        style:
          "background: var(--color-bg-muted); border: 1px solid var(--color-danger); color: var(--color-danger); padding: 12px 16px; border-radius: 12px; margin-bottom: 20px; font-size: 0.875rem; display: flex; align-items: center; gap: 8px;",
      });
      rejBanner.appendChild(el("span", { html: "❌" }));
      rejBanner.appendChild(
        el("span", {
          html: `<strong>Rejection Reason:</strong> ${inv.rejectionReason}`,
        }),
      );
    ...
    if (inv.status === "Pending") {
      const banner = el("div", {
        ...
        style:
          "background: var(--color-bg-muted); border: 1px solid var(--color-warning); color: var(--color-warning); padding: 12px 16px; border-radius: 12px; margin-bottom: 20px; font-size: 0.875rem; display: flex; align-items: center; gap: 8px;",
      });
      banner.appendChild(el("span", { html: "⚠️" }));
      banner.appendChild(
        el("span", {
          text: "This invoice is pending administrative approval and cannot be printed, sent, or have payments recorded until approved.",
        }),
      );
    ...
    if (inv.workRequestId) {
      const linkedWr = window.apiClient.workRequestCache.getById(
        inv.workRequestId,
      );
      if (linkedWr) {
        const linkCard = el("div", {
          style:
            "background:rgba(59,130,246,0.06);border:1px solid rgba(59,130,246,0.15);border-radius: 12px;padding:12px 16px;margin-bottom:var(--spacing-md);font-size:0.8125rem;",
        });
        const linkHeader = el("div", {
          style:
            "display:flex;align-items:center;gap:6px;margin-bottom:6px;color:#1e40af;font-weight:600;",
        });
        linkHeader.appendChild(
          el("span", {
            html: '<svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="#3b82f6" stroke-width="2"><path d="M10 13a5 5 0 007.54.54l3-3a5 5 0 00-7.07-7.07l-1.72 1.71"/><path d="M14 11a5 5 0 00-7.54-.54l-3 3a5 5 0 007.07 7.07l1.71-1.71"/></svg>',
          }),
        );
        ...
        wrLink.addEventListener("mouseenter", () => {
          wrLink.style.textDecoration = "underline";
        });
        wrLink.addEventListener("mouseleave", () => {
          wrLink.style.textDecoration = "none";
        });
        linkCard.appendChild(wrLink);

        if (inv.linkedTaskId) {
          const linkedTask = (linkedWr.tasks || []).find(
            (t) => t.id === inv.linkedTaskId,
          );
          if (linkedTask) {
    ...
    const thr = el("tr");
    ["Type", "Description", "Amount"].forEach((h) =>
    ...
    totals.appendChild(
      el("div", { class: "total-row total-grand" }, [
        el("span", { text: "Total:" }),
        el("span", { text: formatPHP(inv.total) }),
      ]),
    );
    totals.appendChild(
      el("div", { class: "total-row" }, [
        el("span", { text: "Paid:" }),
        el("span", { text: formatPHP(paid) }),
      ]),
    );
    totals.appendChild(
      el("div", { class: "total-row" }, [
        el("span", { text: "Balance:" }),
        el("span", { text: formatPHP(inv.total - paid) }),
      ]),
    );
    ...
    if (Array.isArray(inv.payments) && inv.payments.length > 0) {
      ...
      inv.payments.forEach((p) => {
        const pCard = el("div", {
          class: "card",
          style:
            "margin-bottom:12px; padding:16px; border:1px solid #e2e8f0; border-radius: 12px;",
        });
        ...
        amtBlock.appendChild(
          el("span", {
            text: formatPHP(p.amount),
            style:
              "display:block; font-weight:700; font-size:1.25rem; color:#1e293b; line-height:1.2;",
          }),
        );
        amtBlock.appendChild(
          el("span", {
            ...
            style:
              "display:block; font-size:0.75rem; color:#94a3b8; margin-top:2px;",
          }),
        );
        ...
        pCard.appendChild(
          el("div", {
            style: "height:1px; background:#e2e8f0; margin:0 0 12px;",
          }),
        );
        ...
        const addRow = (label, value) => {
          ...
          const row = el("div", {
            style:
              "display:flex; justify-content:space-between; align-items:baseline; font-size:0.8125rem;",
          });
          row.appendChild(
            el("span", {
              text: label,
          ...
          row.appendChild(
            el("span", {
              text: value,
              style: "color:#334155; font-weight:600; text-align:right;",
            }),
          );
        ...
        const recorder = p.recordedBy
          ? window.apiClient.userCache.getById(p.recordedBy)
          : null;
        const collector = p.collectedBy
          ? window.apiClient.userCache.getById(p.collectedBy)
          : null;
        ...
        if (p.notes) {
          ...
          pCard.appendChild(
            el("div", {
              text: p.notes,
              style:
                "font-size:0.8125rem; color:#64748b; font-style:italic; line-height:1.4;",
            }),
          );
        }
    ...
    const canRecordPayment =
      Auth.can("billing:edit") || Auth.can("billing:mark_paid");
    const isReleased = [
      "Sent",
      "Approved",
      "Partially Paid",
    ...
    if (canRecordPayment && isReleased) {
      ...
      payForm.appendChild(
        el("div", { class: "form-group" }, [
          ...
          el("input", {
            ...
            step: 0.01,
            required: true,
            placeholder: `Balance remaining: ${formatPHP(inv.total - paid)}`,
          }),
        ]),
      );
      payForm.appendChild(
      ...
      methods.forEach((m) =>
        methodSel.appendChild(el("option", { value: m.value, text: m.text })),
      );
      ...
      const bankFields = el("div", {
        class: "pay-bank-fields",
        style: "display:none;",
      });
      bankFields.appendChild(
        createFieldGroup("bankName", "Bank Name *", "text", "e.g., BDO, BPI"),
      );
      bankFields.appendChild(
        createFieldGroup(
          "bankAccount",
      ...
      const digitalFields = el("div", {
        class: "pay-digital-fields",
        style: "display:none;",
      });
      digitalFields.appendChild(
        createFieldGroup(
          "transactionId",
          "Transaction / Reference ID *",
          "text",
      ...
      methodSel.addEventListener("change", () => {
        ...
        digitalFields.style.display = [
          ...
          ? "block"
          : "none";
        cardFields.style.display = ["Credit Card", "Debit Card"].includes(m)
          ? "block"
          : "none";
      });
      ...
      payForm.appendChild(
        el("div", { class: "form-group" }, [
          ...
          el("input", {
            type: "text",
            name: "payRef",
            placeholder: "Any additional reference number",
          }),
        ]),
      );
      ...
      payForm.appendChild(
        el("div", { class: "form-group" }, [
          ...
          el("textarea", {
            name: "payNotes",
            rows: 2,
            placeholder: "e.g., Partial payment, installment #1, etc.",
          }),
        ]),
      );
      ...
      collectorSel.appendChild(
        el("option", { value: "", text: "— Select User —" }),
      );
      ...
      payForm.addEventListener("submit", async (e) => {
        ...
        else if (newPaid > 0 && newPaid < inv.total)
        ...
        try {
          ...
          Workflow.showMessage(
            "Payment Failed",
            e.message || "Unable to record payment.",
            "error",
          );
          return;
        }
    ...
    const birFooter = el("div", {
      class: "bir-footer",
      style:
        "margin-top:40px; padding-top:20px; border-top:2px solid var(--color-border); display:none;",
    });
    birFooter.appendChild(
      el("p", {
        style:
    ...
    if (inv.status === "Draft") {
      ...
      if (canApprove) {
        ...
        approveBtn.addEventListener("click", async () => {
          ...
          const runResult = await Workflow.runBlockingArchiveAction({
            ...
            apiCall: async () => {
              ...
              const res = await window.apiClient.invoices.update(inv.id, {
                status: "Approved",
              });
        ...
        sendBtn.addEventListener("click", async () => {
          ...
          const runResult = await Workflow.runBlockingArchiveAction({
            ...
            successMessage:
              "Invoice has been sent for administrative approval.",
            errorTitle: "Submit Failed",
          });
          if (runResult.success) {
            await PendingChanges.submit(
              "invoices",
              { ...inv, status: "Approved" },
              false,
            );
      ...
      const canReleaseDirectly =
        ...
        Auth.isManagerial() ||
        Auth.can("billing:release");
      const sentBtn = el("button", {
        class: "btn btn-primary",
        text: canReleaseDirectly ? "Release Invoice" : "Submit for Release",
      });
      sentBtn.addEventListener("click", async () => {
        const canReleaseDirectly =
          ...
          Auth.isManagerial() ||
          Auth.can("billing:release");
        const targetStatus = canReleaseDirectly
          ? "Sent"
          : "Release Pending Approval";
        const snapshot = this._updateCachedItem(inv.id, {});
        const runResult = await Workflow.runBlockingArchiveAction({
          ...
          successTitle:
            targetStatus === "Sent" ? "Invoice Released" : "Submitted",
          successMessage:
            targetStatus === "Sent"
    ...
    if (
      ...
      if (inv.status !== "Paid") {
        const routePaidBtn = el("button", {
          class: "btn btn-success",
          text: "Route to Paid Phase (Request Approval)",
          style: "margin-right:8px;",
        });
        routePaidBtn.addEventListener("click", () => {
          Workflow.showConfirm(
            "Request Paid Phase Routing",
            `Submit request to route invoice "${inv.invoiceNumber}" (${formatPHP(inv.total)}) to Paid phase for Admin approval?`,
            () => {
              Workflow.runBlockingArchiveAction({
                title: "Submitting Paid Routing Request",
      ...
      if (inv.status !== "Partially Paid" && inv.status !== "Paid") {
        const routePartialBtn = el("button", {
          class: "btn btn-success",
          text: "Route to Partially Paid Phase (Request Approval)",
          style: "margin-right:8px;",
        });
        routePartialBtn.addEventListener("click", () => {
          Workflow.showConfirm(
            "Request Partially Paid Phase Routing",
            `Submit request to route invoice "${inv.invoiceNumber}" (${formatPHP(inv.total)}) to Partially Paid phase for Admin approval?`,
            () => {
              Workflow.runBlockingArchiveAction({
      ...
      const payRows = inv.payments
        .map((p, idx) => {
          ...
          if (p.method === "Check") {
            ...
            ["GCash", "Maya", "PayPal", "Other Digital"].includes(p.method)
          ) {
            detailRows = `
      ...
      <div class="section">
        ...
        <table>
          ...
          <tbody>
            ...
            <tr><td>22010</td><td>Expanded Withholding Tax Payable (EWT)</td><td class="num">${formatPHP(Math.round(subtotal * 0.1 * 100) / 100)}</td><td class="num">—</td></tr>
  ...
  async renderTemplates() {
    ...
    this.backgroundRefreshTemplates().catch((err) => {
      if (!isAbortError(err))
        console.warn("Billing template background refresh failed", err);
    });
              ...
              Workflow.showConfirm(
                ...
                async () => {
                  await Workflow.runBlockingArchiveAction({
                    ...
                    apiCall: () =>
                      window.apiClient.invoices.deleteTemplate(t.id),
                    ...
                    onAfterConfirm: async () => {
                      if (
                        typeof window.apiClient?.invoices?.invalidateCounts ===
                        "function"
                      ) {
                        window.apiClient.invoices.invalidateCounts();
                      }
      ...
      bulkActions: (selectedIds) => [
        {
          ...
          onClick: (ids) => {
            const title =
              ids.length === 1 ? "Generate Invoice" : "Bulk Generate Invoices";
            const message =
              ids.length === 1
            ...
            Workflow.showConfirm(title, message, async () => {
              ...
              for (const id of ids) {
                ...
                const now = new Date();
                const invoiceNumber =
                  await this.nextInvoiceNumber(recordEntity);
                const subtotal = (t.lineItems || []).reduce(
                  (sum, item) => sum + (parseFloat(item.amount) || 0),
                  0,
                );
                const optimisticId =
              ...
              if (failed.length === 0) {
                Workflow.showMessage(
                  "Success",
                  `Generated ${succeededCount} invoice${succeededCount === 1 ? "" : "s"} successfully.`,
                  "success",
                );
              } else {
                ...
                Workflow.showMessage(
                  "Error",
                  `${failed.length} template(s) could not be generated (${names}).`,
                  "error",
                );
              }
            });
        ...
        },
        {
          ...
          onClick: (ids) => {
            const title =
              ids.length === 1 ? "Delete Template" : "Delete Templates";
            const message =
              ids.length === 1
            ...
            Workflow.showConfirm(
              ...
              async () => {
                try {
                  await Promise.all(
                    ids.map((id) =>
                      window.apiClient.invoices.deleteTemplate(id),
                    ),
                  );
                  this._templates = this._templates.filter(
                    (tm) => !ids.includes(tm.id),
                  );
                  ...
                  Workflow.showMessage(
                    "Delete Failed",
                    e.message || "Unable to delete templates.",
                    "error",
                  );
                  return;
                }
  ...
  async renderTemplateForm(opts = {}) {
    await Promise.all([window.apiClient.clientCache.ensure()]);
          ...
          Workflow.showConfirm(
            ...
            async () => {
              await Workflow.runBlockingArchiveAction({
                ...
                apiCall: () =>
                  window.apiClient.invoices.deleteTemplate(template.id),
                successTitle: "Template Deleted",
    ...
    const titleSection = el("div", {
      class: "notion-freeform notion-freeform--title",
    });
    titleSection.appendChild(
      el("label", { class: "notion-section-label", text: "Template Name" }),
    );
    const nameInput = el("input", {
      type: "text",
      name: "name",
      class: "notion-freeform-input notion-title-input",
      placeholder: "New Billing Template",
      required: true,
      value: template?.name || "",
    });
    ...
    form.addEventListener("submit", (e) => {
      e.preventDefault();
      this.submitTemplateForm(form, template).catch((err) =>
        console.error("submitTemplateForm error", err),
      );
    });
  ...
  async submitTemplateForm(form, template) {
    ...
    const payload = {
      ...
      lineItems: [
        {
          type: "Professional Fee",
          description: fd.get("name").trim(),
          amount: parseFloat(fd.get("pfAmount")) || 0,
        },
      ],
    };
    ...
    await Workflow.runBlockingArchiveAction({
      ...
      apiCall: async () => {
        if (template) {
          return await window.apiClient.invoices.updateTemplate(
            template.id,
            payload,
          );
        } else {
  ...
  async showTemplateForm(existing = null, mode = null) {
    ...
    const fullPageRoute = this.templateEditingId
      ? `#billing/templateForm/${this.templateEditingId}`
      : "#billing/templateForm/new";
    const template = this.templateEditingId
      ? await this.getTemplateById(this.templateEditingId)
  ...
  async generateFromTemplate(t) {
    ...
    const optimisticId =
      "temp-gen-" + Date.now() + "-" + Math.random().toString(36).slice(2);
    const payload = {
      invoiceNumber,
      clientId: t.clientId,
      workRequestId: null,
    ...
    try {
      ...
      Workflow.showMessage(
        "Invoice Success",
        "Generated invoice " + inv.invoiceNumber,
        "success",
      );
    } catch (e) {
      ...
      async () => {
        await this._withArchiveLock(async () => {
          await Workflow.runBlockingArchiveAction({
            ...
            apiCall: () =>
              window.apiClient.invoices.update(id, {
                status: "Cancelled",
                archived: true,
              }),
            ...
            onAfterConfirm: async () => {
              ...
              App.updateSidebarNotifications().catch(() => {});
              if (
                (this.view === "detail" || this.view === "form") &&
                this.detailId === id
              ) {
                location.hash = "#billing";
                return;
  ...
  async restoreInvoice(id) {
    let inv =
      this.getInvoiceById(id) || (this._detailCache && this._detailCache[id]);
  ...
  bulkArchiveInvoices(ids) {
    ...
    if (eligible.length === 0) {
      Workflow.showMessage(
        "No eligible records",
        "Only Paid invoices can be archived.",
        "info",
      );
      return;
    }
          ...
          await Workflow.runBlockingArchiveAction({
            ...
            apiCall: async () => {
              ...
              if (failCount > 0 && successCount === 0) {
                return {
                  error: {
                    message: `${failCount} invoice(s) could not be archived.`,
                  },
                };
              }
  ...
  bulkTrashInvoices(ids) {
    if (!Auth.can("billing:edit")) {
      Workflow.showMessage(
        "Permission Denied",
        "You do not have permission to trash invoices.",
        "danger",
      );
      return;
    }
    ...
    if (eligible.length === 0) {
      Workflow.showMessage(
        "No eligible records",
        "Only Draft invoices can be moved to trash.",
        "info",
      );
      return;
    }
          ...
          await Workflow.runBlockingArchiveAction({
            ...
            apiCall: async () => {
              ...
              if (failCount > 0 && successCount === 0) {
                return {
                  error: {
                    message: `${failCount} invoice(s) could not be moved to trash.`,
                  },
                };
              }
              return { data: { successCount, failCount } };
  ...
  async unarchiveInvoice(id) {
    ...
    Workflow.showConfirm(
      ...
      async () => {
        await this._withArchiveLock(async () => {
          const targetStatus =
            inv.status === "Cancelled" ? "Draft" : inv.status;
          await Workflow.runBlockingArchiveAction({
            ...
            apiCall: async () => {
              ...
              if (inv.status === "Cancelled") {
                const updRes = await window.apiClient.invoices.update(id, {
                  status: "Draft",
                  archived: false,
                });
                res = updRes || res;
  ...
  permanentDeleteInvoice(id) {
    ...
    if (
      Auth.user?.role !== "Admin" &&
      !Auth.isManagerial() &&
      !Auth.can("billing:delete")
    ) {
      Workflow.showMessage(
        "Permission Denied",
        "Only authorized users can permanently delete invoices.",
        "danger",
      );
      return;
    }
    Workflow.showConfirm(
      ...
      async () => {
        try {
          ...
          Workflow.showMessage(
            "Deleted",
            "Invoice has been permanently deleted.",
            "success",
          );
  ...
  async renderArchive() {
    ...
    const localArchived = (this._listCache || []).filter((inv) =>
      this._isArchiveInvoice(inv, entity),
    );
    ...
    archivedInvoices = Array.from(invMap.values()).filter((inv) => {
      ...
      // Exclude only if the cached record is definitely active (not archived and not cancelled).
      return (
        !cached || cached.archived !== false || cached.status === "Cancelled"
      );
    });

    const paid = archivedInvoices.filter(
      (inv) => inv.archived === true && inv.status !== "Cancelled",
    );
    const cancelled = archivedInvoices.filter(
    ...
    try {
      const pendingRes = await window.apiClient.admin.listPendingApprovals({
        status: "rejected",
        tableName: "invoices",
      });
      ...
      return {
        ...
        meta: [
          { icon: ArchivePage.icons.client, text: clientName },
          {
            ...
            text: formatDate(
              pc?.reviewedAt || pc?.updatedAt || r?.requestedAt || r?.updatedAt,
            ),
          },
            ...
            ? [
                {
                  ...
                  onClick: () => {
                    ...
                    else if (r?.workRequestId)
                      location.hash = "#operations/detail/" + r.workRequestId;
                  },
                },
              ]
            : []),
  ...
  async renderAging() {
    ...
    try {
      ...
      Object.entries(bucketMap).forEach(([bucketKey, { keys, label }]) => {
        ...
        grid.appendChild(
          this.kpiCard(label, count + " invoices", formatPHP(total)),
        );
      });
